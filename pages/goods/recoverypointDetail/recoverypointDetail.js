...
Page({
    ...
    detail(lng, lat) {
      ...
      lng = conversion(lng)
      lat = conversion(lat)
      let user_sid = wx.getStorageSync('userInfo').user_sid
      if (!lng && !lat) return;
        http.get('nearby_detail', {
            ...
            success: res => {
                if (res.result == 200) {
                  ...
                  data.admin_name = res.recycle_bin.admins[0]
                ...
                var dataicon = res.recycle_bin.c_products
                for (let i = 0; i < dataicon.length; i++) {
                    dataicon[i].icon = iconUrl + dataicon[i].id
                }
                that.setData({
